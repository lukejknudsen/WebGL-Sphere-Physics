...
inputSpheres = JSON.parse('\
    [\
        {"x": 0.5, "y": 0.5, "z": 1, "r":0.15, "ambient": [0.1,0.1,0.1], "diffuse": [0.0,0.6,0.0], "specular": [0.3,0.3,0.3], "n": 9, "alpha": 0.2, "texture": false, "velocity": [0.0, 0.0, 0.0], "mass": 1},\
        {"x": 0.9, "y": 0.25, "z": 0.5, "r":0.1, "ambient": [0.1,0.1,0.1], "diffuse": [0.6,0.6,0.0], "specular": [0.3,0.3,0.3], "n": 7, "alpha": 0.4, "texture": false, "velocity": [0.01, 0.03, 0.02], "mass": 0.3},\
        {"x": 0.2, "y": 0.25, "z": 0.5, "r":0.05, "ambient": [0.1,0.1,0.1], "diffuse": [0.0,0.0,0.0], "specular": [0.3,0.3,0.3], "n": 7, "alpha": 0.4, "texture": false, "velocity": [0.02, 0.005, 0.02], "mass": 2},\
        {"x": 0.75, "y": 0.5, "z": 0.5, "r":0.07, "ambient": [0.1,0.1,0.1], "diffuse": [0.6,0.0,0.6], "specular": [0.3,0.3,0.3], "n": 7, "alpha": 0.4, "texture": false, "velocity": [0.01, 0.01, 0.02], "mass": 0.3}\
    ]'
);
...
const BOX =   [
  // Left
  {
    "material": {"ambient": [0.1,0.1,0.1], "diffuse": [0.0,0.6,0.6], "specular": [0.3,0.3,0.3], "n": 11, "alpha": 0.9, "texture": false},
  ...
  // Right
  {
    "material": {"ambient": [0.1,0.1,0.1], "diffuse": [0.6,0.0,0.0], "specular": [0.3,0.3,0.3], "n": 11, "alpha": 0.9, "texture": false},
  ...
  // // Front
  // {
  //   "material": {"ambient": [0.1,0.1,0.1], "diffuse": [0.6,0.6,0.6], "specular": [0.3,0.3,0.3], "n": 11, "alpha": 0.9, "texture": false},
...
function handleKeyDown(event) {
    ...
    switch (event.code) {
        ...
        case "Escape": // reset view to default
            ...
            break;
        case "KeyM"://randomize velocities
          for (var whichModel = 0; whichModel < inputSpheres.length; whichModel++) {
            var modelOn = inputSpheres[whichModel];
            modelOn.velocity = [
              Math.random(),
              Math.random(),
              Math.random()
...
function renderModels() {
    ...
    for (var whichModel = 0; whichModel < inputSpheres.length; whichModel++) {
        var modelOn = inputSpheres[whichModel];
        var xMult = modelOn.velocity[0] > 0;
        var yMult = modelOn.velocity[1] > 0;
        var zMult = modelOn.velocity[2] > 0;
        modelOn.acceleration = [airResistance*modelOn.velocity[0], airResistance*modelOn.velocity[1] + gravity, airResistance*modelOn.velocity[2]];
        var temp;
        vec3.dot(temp, modelOn.velocity, modelOn.velocity);
        if (modelOn.y + modelOn.translation[1] - modelOn.r >= 0.0) {
          vec3.add(modelOn.velocity, modelOn.velocity, modelOn.acceleration);
        }
        else {
          // modelOn.velocity[1]=0;
        }

        // It's collision times bois
        for (var collisionIndex = 0; collisionIndex < inputSpheres.length; collisionIndex++) {
          var collisionOn = inputSpheres[collisionIndex];
          modelOnLocation = [modelOn.x+modelOn.translation[0], modelOn.y+modelOn.translation[1], modelOn.z+modelOn.translation[2]];
          collisionOnLocation = [collisionOn.x+collisionOn.translation[0], collisionOn.y+collisionOn.translation[1], collisionOn.z+collisionOn.translation[2]];
          var dist = vec3.dist(modelOnLocation,collisionOnLocation);
          if (collisionIndex != whichModel && dist < modelOn.r + collisionOn.r) {
            // console.log('collision!!!');
            collision++;
            var temp = [0, 0, 0];
            // x
            temp[0] = (2*collisionOn.mass*collisionOn.velocity[0] + (modelOn.mass - collisionOn.mass)*modelOn.velocity[0])/(collisionOn.mass + modelOn.mass);
            collisionOn.velocity[0] = (2*modelOn.mass*modelOn.velocity[0] + (collisionOn.mass - modelOn.mass)*collisionOn.velocity[0])/(modelOn.mass + collisionOn.mass);
            modelOn.velocity[0] = temp[0];
            // y
            temp[1] = (2*collisionOn.mass*collisionOn.velocity[1] + (modelOn.mass - collisionOn.mass)*modelOn.velocity[1])/(collisionOn.mass + modelOn.mass);
            collisionOn.velocity[1] = (2*modelOn.mass*modelOn.velocity[1] + (collisionOn.mass - modelOn.mass)*collisionOn.velocity[1])/(modelOn.mass + collisionOn.mass);
            modelOn.velocity[1] = temp[1];
            // z
            temp[2] = (2*collisionOn.mass*collisionOn.velocity[2] + (modelOn.mass - collisionOn.mass)*modelOn.velocity[2])/(collisionOn.mass + modelOn.mass);
            collisionOn.velocity[2] = (2*modelOn.mass*modelOn.velocity[2] + (collisionOn.mass - modelOn.mass)*collisionOn.velocity[2])/(modelOn.mass + collisionOn.mass);
            modelOn.velocity[2] = temp[2];
            /*
            var twoMV = [0,0,0], diffMassV = [0,0,0];
            vec3.scale(twoMV, collisionOn.velocity, 2*collisionOn.mass);
            vec3.scale(diffMassV, collisionOn.velocity, modelOn.mass - collisionOn.mass);
            vec3.add(temp, twoMV, diffMassV);
            vec3.scale(temp, temp, 1/(collisionOn.mass + modelOn.mass));

            vec3.scale(twoMV, modelOn.velocity, 2*modelOn.mass);
            vec3.scale(diffMassV, modelOn.velocity, collisionOn.mass - modelOn.mass);
            vec3.add(collisionOn.velocity, twoMV, diffMassV);
            vec3.scale(collisionOn.velocity, collisionOn.velocity, 1/(collisionOn.mass + modelOn.mass));
            modelOn.velocity = temp;
            */
          }
        }
        if (collision < 10 && collision != 0) {
          collision++;
        }
        if (collision === 10) {
          collision = 0;
        ...
        var velocity = modelOn.velocity;
        var temp;

        // left/right
        var offset = vec3.scale(temp,viewRight,-velocity[0]);
        vec3.add(modelOn.translation,modelOn.translation,offset);

        // up/down
        var offset = vec3.scale(temp,Up,velocity[1]);
        vec3.add(modelOn.translation,modelOn.translation,offset);

        // forward/backward
        var offset = vec3.scale(temp,lookAt,velocity[2]);
        vec3.add(modelOn.translation,modelOn.translation,offset);

        // bounce off walls
        if (modelOn.x + modelOn.translation[0] + modelOn.r > 1.0 && modelOn.velocity[0] > 0) { // bounce off right side
          modelOn.velocity[0] = -modelOn.velocity[0];
        }
        if (modelOn.x + modelOn.translation[0] - modelOn.r < 0.0 && modelOn.velocity[0] < 0) { // bounce off left side
          modelOn.velocity[0] = -modelOn.velocity[0];
        }
        if (modelOn.y + modelOn.translation[1] + modelOn.r > 1.0 && modelOn.velocity[1] > 0) { // bounce off right side
          modelOn.velocity[1] = -modelOn.velocity[1];
        }
        if (modelOn.y + modelOn.translation[1] - modelOn.r < 0.0 && modelOn.velocity[1] < 0) { // bounce off left side
          modelOn.velocity[1] = -modelOn.velocity[1];
        }
        if (modelOn.z + modelOn.translation[2] + modelOn.r > 1.0 && modelOn.velocity[2] > 0) { // bounce off right side
          modelOn.velocity[2] = -modelOn.velocity[2];
        }
        if (modelOn.z + modelOn.translation[2] - modelOn.r < 0.0 && modelOn.velocity[2] < 0) { // bounce off left side
          modelOn.velocity[2] = -modelOn.velocity[2];
        }
    }

    // // highlightModel(modelEnum.SPHERE,(handleKeyDown.whichOn+1) % numSpheres); does the following
    // var whichModel = 0;
    // var modelOn = inputSpheres[whichModel];
    // // translateModel(vec3.scale(temp,lookAt,viewDelta)); does the faollowing
    // var temp;
    // if (zoomCounter % 500 < 250) {
    //     var offset = vec3.scale(temp,lookAt,-.005);//forward
    // }
    // else {
    //     var offset = vec3.scale(temp,lookAt,.005); //backward
    // }
    // vec3.add(modelOn.translation,modelOn.translation,offset);
    // var whichModel = 1;
    // var modelOn = inputSpheres[whichModel];
    // if (zoomCounter % 100 < 50) {
    //     var offset = vec3.scale(temp,viewRight,.01);//right
    // }
    // else {
    //     var offset = vec3.scale(temp,viewRight,-.01); //left
    // }
    // vec3.add(modelOn.translation,modelOn.translation,offset);
